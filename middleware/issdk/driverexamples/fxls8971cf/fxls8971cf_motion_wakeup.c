...
/**
 * @file  fxls8971_motion_wakeup.c
 *  @brief The fxls8971_motion_wakeup.c file implements the ISSDK FXLS8971CF I2C sensor driver
...
#define FXLS8971_STANDBY_MODE   0
#define FXLS8971_ACTIVE_MODE    1
//-----------------------------------------------------------------------
// Constants
//-----------------------------------------------------------------------
/*! @brief Register settings for configuring SDCD-OT for tap-detection and Auto-Wake/Sleep in interrupt mode. */
const registerwritelist_t cFxls8971AwsConfig[] = {
    ...
    {FXLS8971_SENS_CONFIG3, FXLS8971_SENS_CONFIG3_WAKE_ODR_400HZ | FXLS8971_SENS_CONFIG3_SLEEP_ODR_6_25HZ, FXLS8971_SENS_CONFIG3_WAKE_ODR_MASK | FXLS8971_SENS_CONFIG3_SLEEP_ODR_MASK},
    /* Enable SDCD OT for all 3 axes X, Y & Z and within-thresholds event latch disabled. */
    {FXLS8971_SDCD_CONFIG1, FXLS8971_SDCD_CONFIG1_X_OT_EN_EN | FXLS8971_SDCD_CONFIG1_Y_OT_EN_EN | FXLS8971_SDCD_CONFIG1_Z_OT_EN_EN | FXLS8971_SDCD_CONFIG1_OT_ELE_DIS,
    		FXLS8971_SDCD_CONFIG1_X_OT_EN_MASK | FXLS8971_SDCD_CONFIG1_Y_OT_EN_MASK | FXLS8971_SDCD_CONFIG1_Z_OT_EN_MASK | FXLS8971_SDCD_CONFIG1_OT_ELE_MASK},
    /* Enabling SDCD and Relative Data (N) � Data (N-1) mode for transient detection */
    {FXLS8971_SDCD_CONFIG2, FXLS8971_SDCD_CONFIG2_SDCD_EN_EN | FXLS8971_SDCD_CONFIG2_REF_UPDM_SDCD_REF, FXLS8971_SDCD_CONFIG2_SDCD_EN_MASK | FXLS8971_SDCD_CONFIG2_REF_UPDM_MASK},
    /* Set the SDCD_OT debounce count to 0 */
    {FXLS8971_SDCD_OT_DBCNT, 0, 0},
    /* Set the SDCD lower and upper thresholds to +/-100mg*/
    {FXLS8971_SDCD_LTHS_LSB, 0xCC, 0},
    {FXLS8971_SDCD_LTHS_MSB, 0xFF, 0},
    {FXLS8971_SDCD_UTHS_LSB, 0x34, 0},
    {FXLS8971_SDCD_UTHS_MSB, 0x00, 0},
    /* Enable SDCD outside of thresholds event Auto-WAKE/SLEEP transition source enable. */
    {FXLS8971_SENS_CONFIG4, FXLS8971_SENS_CONFIG4_WK_SDCD_OT_EN | FXLS8971_SENS_CONFIG4_INT_POL_ACT_HIGH, FXLS8971_SENS_CONFIG4_WK_SDCD_OT_MASK | FXLS8971_SENS_CONFIG4_INT_POL_MASK},
    /* Set the ASLP count to 5sec */
    {FXLS8971_ASLP_COUNT_LSB, 0xD0, 0},
    {FXLS8971_ASLP_COUNT_MSB, 0x07, 0},
    /* Enable Interrupts for WAKE mode. */
    {FXLS8971_INT_EN, FXLS8971_INT_EN_WAKE_OUT_EN_EN, FXLS8971_INT_EN_WAKE_OUT_EN_MASK},
    {FXLS8971_INT_PIN_SEL, FXLS8971_INT_PIN_SEL_WK_OUT_INT2_DIS, FXLS8971_INT_PIN_SEL_WK_OUT_INT2_MASK},
    __END_WRITE_DATA__};

/*! @brief Read register list to read SysMode Register. */
const registerreadlist_t Fxls8971ReadSysMode[] = {{.readFrom = FXLS8971_SYS_MODE, .numBytes = 1}, __END_READ_DATA__};

/*! @brief Read register list to read INT_STATUS Register. */
const registerreadlist_t Fxls8971ReadIntStatus[] = {{.readFrom = FXLS8971_INT_STATUS, .numBytes = 1}, __END_READ_DATA__};
 ...
 *  -----------------------------------------------------------------------*/
void fxls8971_int_callback(void *pUserData)
...
int main(void)
{
    ...
    PRINTF("\r\n ISSDK FXLS8971CF sensor driver example to detect motion event & AWS\r\n");

    /*! Initialize FXLS8971 pin used by FRDM board */
    pGpioDriver->pin_init(&FXLS8971_INT1, GPIO_DIRECTION_IN, NULL, &fxls8971_int_callback, NULL);
    ...
    status = FXLS8971_I2C_Configure(&fxls8971Driver, cFxls8971AwsConfig);
    ...
    for (;;) /* Forever loop */
    {
        ...
        status = FXLS8971_I2C_ReadData(&fxls8971Driver, Fxls8971ReadSysMode, &eventStatus);
        if (ARM_DRIVER_OK != status)
        {
            return status;
        }

        if (eventStatus == FXLS8971_SYS_MODE_SYS_MODE_WAKE)
        {
        ...
        else
        {
           if ((waketosleep == 1) || (firsttransition == 1))
           {
             ...
             status = FXLS8971_I2C_ReadData(&fxls8971Driver, Fxls8971ReadIntStatus, &intStatus);
             if (ARM_DRIVER_OK != status)
             {
               return status;
             }
