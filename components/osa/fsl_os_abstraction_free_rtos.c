...
#if (defined(configSUPPORT_STATIC_ALLOCATION) && (configSUPPORT_STATIC_ALLOCATION > 0U))
    xSemaHandle.sem = xSemaphoreCreateCountingStatic(0xFF, initValue, (StaticQueue_t *)((uint8_t *)semaphoreHandle + sizeof(osa_semaphore_handle_t)));
#else
...
#if (defined(configSUPPORT_STATIC_ALLOCATION) && (configSUPPORT_STATIC_ALLOCATION > 0U))
    xSemaHandle.sem = xSemaphoreCreateBinaryStatic((StaticQueue_t *)((uint8_t *)semaphoreHandle + sizeof(osa_semaphore_handle_t)));    
#else
...
#if (defined(configSUPPORT_STATIC_ALLOCATION) && (configSUPPORT_STATIC_ALLOCATION > 0U))
    xMutexHandle.mutex = xSemaphoreCreateRecursiveMutexStatic((StaticQueue_t *)((uint8_t *)mutexHandle + sizeof(osa_mutex_handle_t)));
#else
...
#if (defined(configSUPPORT_STATIC_ALLOCATION) && (configSUPPORT_STATIC_ALLOCATION > 0U))
    pEventStruct->handle = xEventGroupCreateStatic((StaticEventGroup_t *)((uint8_t *)(eventHandle) + sizeof(StaticEventGroup_t)));
#else    
...
osa_status_t OSA_MsgQCreate(osa_msgq_handle_t msgqHandle, uint32_t msgNo, uint32_t msgSize)
{
    ...
    xMsgqHandle.msgq = xQueueCreateStatic(msgNo, msgSize, 
                                          ...
                                          (StaticQueue_t *)((uint8_t *)msgqHandle + sizeof(osa_msgq_handle_t)));
#else
