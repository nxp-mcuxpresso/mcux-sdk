 ...
 * Beware: certain SPIFI implementations (such as that of JN5189/QN9090/K32W061)
 * require that the data do not exceed the actual size of the command, so cannot call
 * SPIFI_WriteData when less than 32 bits are expected.
 *
 * @param base     SPIFI peripheral base address.
 ...
 *
 * @param base     SPIFI peripheral base address.
 * @param data     8 bit word that need to be written.
 */
static inline void SPIFI_Write8bitWord(SPIFI_Type *base, uint8_t byte)
{
...
/*!
 * @brief Write a short data in address of SPIFI.
 *
 * Used for transaction requiring less than 32 bits of data
 *
 * @param base     SPIFI peripheral base address.
 * @param word     16 bit word  that needs to be written.
 */
static inline void SPIFI_Write16bitWord(SPIFI_Type *base, uint16_t word)
{
    __IO uint16_t *reg16 = (__IO uint16_t *) &base->DATA;
    *reg16 = (uint16_t)word;
}
...
/*!
 * @brief Write a buffer worth of data to SPIFI .
 *
 * Used for transaction requiring less than 32 bits of data
 *
 * @param base    SPIFI peripheral base address.
 * @param buf     pointer on octet buffer to be written to the SPIFI.
 * @param size_to_write     size of buffer.
 */
 ...
 *
 * Users should notice before call this function, the data length field in command register shall be larger
 * than 4, otherwise a hard fault will happen.
 *
 * @param base     SPIFI peripheral base address.
...
#ifdef CPU_JN518X
/*!
 * @brief Read an 8 bit word from  SPIFI.
 *
 * Used for transaction requiring less than 32 bits of data
 *
 * @param base     SPIFI peripheral base address.
 */
static inline uint8_t SPIFI_Read8bitWord(SPIFI_Type *base)
{
    __IO uint8_t *reg8 = (__IO uint8_t *) &base->DATA;
    return *reg8;;
}
...
/*!
 * @brief Read between 16 bit word  from SPIFI.
 *
 * Used for transaction requiring less than 32 bits of data
 *
 * @param base     SPIFI peripheral base address.
 */
static inline uint16_t SPIFI_Read16bitWord(SPIFI_Type *base)
{
    __IO uint16_t *reg16 = (__IO uint16_t *) &base->DATA;
    return *reg16;
}
