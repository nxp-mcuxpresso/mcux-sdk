...
void I2C_SlaveClearStatusFlags(I2C_Type *base, uint32_t statusFlags)
{
    ...
    if ((statusFlags & (uint32_t)(kI2C_SlaveFlushTxFifoFlag | kI2C_SlaveArbitrationLostFlag |
                                  kI2C_SlaveReadWhenRequestedDataFlag)) != 0U)
    {
        statusFlags |= (uint32_t)kI2C_TxAbortInterruptFlag;
    }
...
void I2C_MasterSetSpeedMode(I2C_Type *base, i2c_master_speed_mode_t speed, uint32_t sourceClock_Hz)
{
    ...
    {
        highCount            = (5U * sourceClock_Hz / 100000U + 5U) / 10U;
        lowCount             = (5U * sourceClock_Hz / 100000U + 5U) / 10U;
        base->IC_SS_SCL_HCNT = (uint32_t)highCount;
        base->IC_SS_SCL_LCNT = (uint32_t)lowCount;
    }
    else if (speed == kI2C_MasterSpeedFast)
    {
        highCount            = (sourceClock_Hz / 100000U + 5U) / 10U;
        lowCount             = (15U * sourceClock_Hz / 1000000U + 5U) / 10U;
        base->IC_FS_SCL_HCNT = (uint32_t)highCount;
        base->IC_FS_SCL_LCNT = (uint32_t)lowCount;
    }
    else
    {
        highCount            = (4U * sourceClock_Hz / 1000000U + 5U) / 10U;
        lowCount             = (6U * sourceClock_Hz / 1000000U + 5U) / 10U;
...
status_t I2C_MasterCheckAndClearError(I2C_Type *base, uint32_t status)
{
    ...
    if (0U != status)
    {
        ...
        else if (0U != (status & (uint32_t)(kI2C_Master7bitAddressNackFlag | kI2C_Master10bitAddressNack1Flag |
...
status_t I2C_MasterReadBlocking(I2C_Type *base, uint8_t *data, size_t rxSize)
{
    ...
    while (txSize != 0U)
    {
        ...
        txCountEach = MIN((I2C_GetTxFifoDepth(base) - I2C_GetTxFifoDataCount(base)), txSize);
        txSize -= txCountEach;
        ...
        rxCountEach = MIN(I2C_GetRxFifoDataCount(base), rxSize);
        rxSize -= rxCountEach;
...
status_t I2C_MasterTransferNonBlocking(I2C_Type *base, i2c_master_handle_t *handle, i2c_master_transfer_t *transfer)
{
    ...
    I2C_EnableInterrupts(base, (uint16_t)(kI2C_TxRequestInterruptEnable | kI2C_TxAbortInterruptEnable));
...
static void I2C_MasterInitTransferStateMachine(I2C_Type *base, i2c_master_handle_t *handle)
{
    ...
    {
        handle->state = kMasterSendCommandState;
    }
    else if (handle->transfer.direction == kI2C_Write)
    {
        handle->state = kMasterSendDataState;
    }
    else
    {
        handle->state = kMasterReceiveDataState;
    }
}
...
static status_t I2C_MasterRunTransferStateMachine(I2C_Type *base, i2c_master_handle_t *handle, bool *isDone)
{
    ...
    uint8_t txCount;
    uint8_t rxCount             = MIN(I2C_GetRxFifoDataCount(base), handle->receiveSize);
    ...
    *isDone = false;
    ...
    {
        case (uint8_t)kMasterSendCommandState:
            txCount = MIN((I2C_GetTxFifoDepth(base) - txFifoCount), xfer->subaddressSize);
            while (txCount-- != 0U)
            {
                xfer->subaddressSize--;
                ...
                if (0U != xfer->dataSize)
                {
                    if (xfer->direction == kI2C_Read)
                    {
                        ...
                        I2C_EnableInterrupts(base,
                                             (uint16_t)(kI2C_RxOverflowInterruptFlag | kI2C_RxDataReadyInterruptFlag));
                    }
                    else
                    {
        ...
        case (uint8_t)kMasterSendDataState:
            txCount = MIN((I2C_GetTxFifoDepth(base) - txFifoCount), handle->transmitSize);
            while (txCount-- != 0U)
            {
                handle->transmitSize--;
