...
static void I2C_MasterTransferCallbackDMA(dmac_handle_t *handle, void *userData, uint32_t status)
{
    ...
    I2C_DisableInterrupts(i2cDmacHandle->base, (uint16_t)kI2C_AllInterruptEnable);

    /* Wait for transfer to finish then clear all alearable status flags. */
    while ((I2C_MasterGetStatusFlags(i2cDmacHandle->base) & kI2C_StopDetectInterruptFlag) == 0U)
    {
    }

    I2C_MasterClearStatusFlags(i2cDmacHandle->base, (uint32_t)kI2C_MasterAllClearFlags);

    if (i2cDmacHandle->completionCallback != NULL)
    {
        i2cDmacHandle->completionCallback(i2cDmacHandle->base, i2cDmacHandle, kStatus_Success, i2cDmacHandle->userData);
    }
}
...
status_t I2C_MasterTransferDMAC(I2C_Type *base, i2c_master_dmac_handle_t *handle, i2c_master_transfer_t *xfer)
{
    ...
    {
        DMAC_GetChannelDefaultTransferConfig(&dmacConfig, (void *)xfer->data, (uint32_t *)I2C_GetDataRegAddr(base),
        ...
        DMAC_TransferSubmit(handle->dmacTxHandle, &dmacConfig);
    }
    else
    {
        ...
        DMAC_TransferSubmit(handle->dmacRxHandle, &dmacConfig);
