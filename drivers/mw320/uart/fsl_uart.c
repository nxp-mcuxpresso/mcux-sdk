...
void UART_SetAutoBaud(UART_Type *base)
{
    ...
    /* Configure the Baud Rate */
    base->LCR |= UART_LCR_DLAB_MASK;
    base->DLH = (autoBaudCount >> 12U) & 0xFF;
    base->DLL = (autoBaudCount >> 4U) & 0xFF;
    base->LCR &= ~UART_LCR_DLAB_MASK;
}
...
uint16_t UART_Read9bits(UART_Type *base)
{
    ...
    parity ^= (base->LSR & UART_LSR_PE_MASK) >> UART_LSR_PE_SHIFT;
...
status_t UART_TransferSendNonBlocking(UART_Type *base, uart_handle_t *handle, uart_transfer_t *xfer)
{
    ...
    else
    {
        ...
        UART_EnableInterrupts(base, (uint16_t)kUART_TxDataRequestInterruptEnable);
...
void UART_TransferAbortSend(UART_Type *base, uart_handle_t *handle)
{
    ...
    UART_DisableInterrupts(base, (uint16_t)kUART_TxDataRequestInterruptEnable);
