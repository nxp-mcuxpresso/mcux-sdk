...
#if defined(FSL_FEATURE_FMEAS_STARTMEAS_SCALE) && (FSL_FEATURE_FMEAS_STARTMEAS_SCALE)
/*!
 * @brief    Starts a frequency measurement cycle with specific time.
 *
 * @param    base : SYSCON peripheral base address.
 * @param    scale : measurement time is 2^scale cycle of reference clock, value is from 2 to 31.
 */
static inline void FMEAS_StartMeasureWithScale(FMEAS_SYSCON_Type *base, uint8_t scale)
{
    assert((scale >= 2) && (scale <= 31));
    ...
    base->FREQMECTRL = scale | FMEAS_SYSCON_FREQMECTRL_PROG_MASK;
}
#endif /*FSL_FEATURE_FMEAS_STARTMEAS_SCALE*/
